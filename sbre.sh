...
add_rule_menu() {
    ...
    # AI服务额外叠加domain_suffix补充规则(见 extra_ai_domains 说明)，跟rule_set是两条独立规则、互不影响
    ...
    if [ -n "$extra_domains_json" ]; then
        if ! jq -e --argjson d "$extra_domains_json" --arg out "$selected_out" \
            '.route.rules[] | select(.domain_suffix != null and .outbound == $out and (.domain_suffix == $d))' \
            "$route_file" > /dev/null 2>&1; then
            jq --argjson d "$extra_domains_json" --arg out "$selected_out" \
                '.route.rules += [{"domain_suffix": $d, "outbound": $out}]' \
                "$route_file" > "${route_file}.tmp" && mv "${route_file}.tmp" "$route_file"
        fi
    fi
