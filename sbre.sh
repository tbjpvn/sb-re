...
# SS2022分流修复版：支持v2rayN导出的SS2022 URI；按实际PSK长度自动校正AES-128/AES-256；
# 添加SS2022出站时不覆盖现有分流规则，并在写入后执行sing-box配置检查，失败自动回滚。
# =========================
...
# 申请/切换域名证书，可传入域名字符串；传空则恢复自签证书
# 成功后：写入 ${work_dir}/domain.txt，并设置全局变量 fingerprint(自签时用)
issue_domain_cert() {
    local domain="$1"
...
#!/usr/bin/env bash
bash <(curl -Ls https://raw.githubusercontent.com/tbjpvn/sb-re/main/sbre.sh) $1
EOF
...
# 确保路由在所有规则之前执行协议/域名嗅探。
# 现代 sing-box 推荐使用 route.rules 的 action: sniff，
# 而不是旧版 inbound.sniff / sniff_override_destination。
ensure_sniff_rule() {
    ...
    jq '
      .route.rules = ([.route.rules[]? | select(
        (.action? == "sniff") or
        (.rule_set? != null and (.rule_set | length) > 0) or
        (.domain_suffix? != null) or
        (.domain? != null) or
        (.ip_cidr? != null) or
        (.ip_is_private? != null) or
        (.protocol? != null) or
        (.network? != null) or
        (.inbound? != null)
      )]) |
      if any(.route.rules[]?; .action? == "sniff")
      then .
      else .route.rules = [{"action":"sniff"}] + .route.rules
      end
...
# 迁移/规范化旧版 route 规则：
# sing-box 1.11+ 已将规则的 outbound 放入 action 中。
# 老王脚本历史上使用：
#   {"rule_set":["gemini"],"outbound":"relay"}
# 现代写法应为：
#   {"rule_set":["gemini"],"action":"route","outbound":"relay"}
    ...
    jq '
      .route.rules = [
        ...
        if (.action? == "sniff") then
          .
        elif (.outbound? != null and .action? == null) then
          .action = "route"
        else
          .
        end
...
# 检查并应用 route.json；失败时恢复备份，避免 sing-box 整体掉线。
apply_route_with_rollback() {
    local backup="$1"
    local check_output restart_rc=0

    check_output=$("${work_dir}/${server_name}" check -C "$conf_dir" 2>&1)
    if [ $? -ne 0 ]; then
        red "sing-box 路由配置检查失败："
        echo "$check_output"
        cp -f "$backup" "${conf_dir}/route.json"
        red "已恢复修改前的 route.json，未重启 sing-box。"
        return 1
    fi

    restart_singbox >/dev/null 2>&1
    if command_exists systemctl; then
        systemctl is-active --quiet sing-box || restart_rc=1
    elif command_exists rc-service; then
        rc-service sing-box status >/dev/null 2>&1 || restart_rc=1
    fi

    if [ "$restart_rc" -ne 0 ]; then
        red "sing-box 重启后未正常运行，正在恢复修改前的 route.json..."
        cp -f "$backup" "${conf_dir}/route.json"
...
warp_manage() {
    ...
    jq -r '.route.rules[] | select(.rule_set != null) | .rule_set[]?' "$route_file" 2>/dev/null | sort -u | while read tag; do
        extra=$(extra_ai_domains "$tag")
        if [ -n "$extra" ]; then
            echo -e " - ${skyblue}$tag${re} ${yellow}(已叠加API域名补充规则)${re}"
        else
            echo -e " - ${skyblue}$tag${re}"
        fi
    done || echo "  无"
    green "\n已添加的代理出站(socks/http/ss):"
    ...
    red "2. 删除分流服务"
    skyblue "--------------"
    green "3. 添加代理出站 (支持 socks5/http/ss2022)"
    skyblue "----------------------"
    red "4. 删除代理出站"
    skyblue "----------------------"
    purple "0. 返回主菜单"
...
extra_ai_domains() {
    case "$1" in
        gemini)
            echo '["gemini.google.com","generativelanguage.googleapis.com","proactivebackend-pa.googleapis.com","alkalimakersuite-pa.clients6.google.com","aistudio.google.com","makersuite.google.com","aitestkitchen.withgoogle.com","robinfrontend-pa.googleapis.com","optimizationguide-pa.googleapis.com"]'
            ;;
        openai)
            echo '["openai.com","chatgpt.com","chat.openai.com","oaistatic.com","oaiusercontent.com","ai.com"]'
            ;;
        claude)
            echo '["claude.ai","anthropic.com","claude.com"]'
            ;;
        *)
            echo ''
            ;;
...
add_rule_menu() {
    ...
    local out_tags=()
    mapfile -t out_tags < <(
        jq -r '.outbounds[]? | select(.tag != "direct") | .tag' \
    ...
    if ! jq -e --arg out "$selected_out" \
        '.outbounds[]? | select(.tag == $out)' "$outbound_file" >/dev/null 2>&1; then
        red "出站 '${selected_out}' 不存在，已取消。"
        sleep 2
        warp_manage
        return
    fi
    ...
    local route_backup
    route_backup="$(mktemp)"
    cp -f "$route_file" "$route_backup" || {
        red "无法备份 route.json，已取消。"
        rm -f "$route_backup"
        sleep 2
        warp_manage
        return
    ...
    # 使用现代 sing-box 规则格式：
    # sniff 是非最终 action；真正分流使用 action=route + outbound。
    if ! jq --arg tag "$rule_tag" --arg out "$selected_out" '
        .route.rules += [{
            "rule_set": [$tag],
            "action": "route",
            "outbound": $out
        }]
    ' "$route_file" > "${route_file}.tmp"; then
        red "写入分流规则失败。"
        rm -f "$route_backup" "${route_file}.tmp"
        sleep 2
        warp_manage
        return
    fi
    ...
    extra_domains_json=$(extra_ai_domains "$rule_tag")
    if [ -n "$extra_domains_json" ]; then
        if ! jq -e --argjson d "$extra_domains_json" --arg out "$selected_out" \
            '.route.rules[]? |
             select(.domain_suffix != null and .outbound == $out and (.domain_suffix == $d))' \
            "$route_file" >/dev/null 2>&1; then

            if ! jq --argjson d "$extra_domains_json" --arg out "$selected_out" \
                '.route.rules += [{
                    "domain_suffix": $d,
                    "action": "route",
                    "outbound": $out
                }]' "$route_file" > "${route_file}.tmp"; then
                red "写入 AI 补充域名规则失败，正在回滚。"
                cp -f "$route_backup" "$route_file"
                rm -f "$route_backup" "${route_file}.tmp"
                sleep 2
                warp_manage
                return
    ...
    if apply_route_with_rollback "$route_backup"; then
        rm -f "$route_backup"
        green "'${rule_tag}' 已分流至出站 '${selected_out}'"
        green "当前 route.json 使用现代 action=route + outbound 格式。"
        sleep 2
        warp_manage
    else
        rm -f "$route_backup"
...
delete_rule_menu() {
    ...
    jq --arg tag "$tag" \
       'del(.route.rules[] | select(.rule_set != null) | .rule_set[] | select(. == $tag)) |
        .route.rules = [.route.rules[] | select(.action == "sniff" or (.rule_set != null and (.rule_set | length) > 0) or .domain_suffix != null or .domain != null or .ip_cidr != null or .ip_is_private != null or .protocol != null or .network != null or .inbound != null)] |
        if any(.route.rules[]?; .action == "sniff") then . else .route.rules = [{"action":"sniff"}] + .route.rules end' \
       "$route_file" > "${route_file}.tmp" && mv "${route_file}.tmp" "$route_file"

    # 同步删除该服务的domain_suffix补充规则(如果有)
    extra_domains_json=$(extra_ai_domains "$tag")
    if [ -n "$extra_domains_json" ]; then
        jq --argjson d "$extra_domains_json" \
           '.route.rules = [.route.rules[] | select(.domain_suffix == null or (.domain_suffix != $d))]' \
           "$route_file" > "${route_file}.tmp" && mv "${route_file}.tmp" "$route_file"
    fi
...
# 解析 ss:// (含ss2022) 链接，添加为 shadowsocks 出站，用于把指定域名流量转发给另一台机子的ss2022节点
add_ss_outbound() {
    local proxy_url="$1"
    local after_proto tag_from_url userinfo host_port server port method password
    local decoded_file raw_key_file raw_userinfo expected_len actual_len normalized_method
    local backup_outbound backup_route check_output restart_status

    after_proto="${proxy_url#ss://}"

    # 处理 URL fragment/tag，并去掉可能存在的插件参数
    if [[ "$after_proto" == *"#"* ]]; then
    ...
    [[ "$port" =~ ^[0-9]+$ ]] || { red "格式错误：端口无效"; sleep 2; return; }
    ...
    # SS / SS2022 链接解析
    #
    # 普通 SS：userinfo 通常是 base64(method:password)
    # SS2022：userinfo 解码后为：method + ':' + 原始二进制 PSK
    #
    # 不能把 SS2022 的二进制 PSK 直接塞进 Bash 变量再交给 jq，
    # 否则会经过 UTF-8/命令替换破坏二进制数据，导致：
    #   decode key: illegal base64 data at input byte 0
    #
    # sing-box 的 SS2022 password 使用 PSK 的 Base64 表示，因此这里
    # 始终从临时二进制文件提取 PSK，再重新 Base64 编码。
    # ============================================================
    decoded_file="$(mktemp)"
    raw_key_file="$(mktemp)"

    if [[ "$userinfo" == *":"* ]]; then
        # 明文 method:password 形式，主要兼容部分客户端导出的链接。
        # 对 SS2022，如果 password 看起来是 Base64，则按 Base64 PSK 尝试；
        # 如果不是，则按普通字符串处理。
        method="${userinfo%%:*}"
        password="${userinfo#*:}"

        if [[ "$method" == 2022-* ]]; then
            # 明文形式的 SS2022 一般 password 本身就是 Base64 PSK。
            # 如果无法解码，则拒绝写入，避免产生坏配置。
            if ! printf '%s' "$password" | base64 -d > "$raw_key_file" 2>/dev/null; then
                red "SS2022 密钥不是有效的 Base64 格式。"
                sleep 2
                rm -f "$decoded_file" "$raw_key_file"
                return
            fi
        fi
    else
        # SIP002：base64(method:password)
        # 同时兼容 URL-safe Base64 和缺少 '=' padding 的导出格式。
        raw_userinfo="$userinfo"
        raw_userinfo="${raw_userinfo//-/+}"
        raw_userinfo="${raw_userinfo//_/\/}"
        while (( ${#raw_userinfo} % 4 != 0 )); do
            raw_userinfo+='='
        done

        if ! printf '%s' "$raw_userinfo" | base64 -d > "$decoded_file" 2>/dev/null; then
            red "无法解析 SS 节点用户信息：Base64 格式错误。"
            sleep 2
            rm -f "$decoded_file" "$raw_key_file"
            return
        fi

        # 二进制安全地提取第一个 ':' 前的 method。
        # method 只包含 ASCII，因此这里用 awk/sed 不会影响后面的 PSK。
        method="$(LC_ALL=C awk 'BEGIN{RS=""} {p=index($0,":"); if(p>0) printf "%s", substr($0,1,p-1)}' "$decoded_file")"
        [ -z "$method" ] && { red "无法解析 SS 节点加密方式。"; sleep 2; return; }

        if [[ "$method" == 2022-* ]]; then
            # 不通过 Bash 变量保存二进制 PSK。
            # 直接从 method 后面的原始字节提取密钥。
            method_len=${#method}
            # method 为 ASCII，长度可以安全用于字节偏移。
            dd if="$decoded_file" of="$raw_key_file" bs=1 skip=$((method_len + 1)) status=none 2>/dev/null
            actual_len=$(wc -c < "$raw_key_file")

            [ "$actual_len" -gt 0 ] || { red "SS2022 节点没有有效的 PSK。"; sleep 2; return; }

            # SS2022 的 PSK 长度决定真正的算法：
            # AES-128-GCM = 16 bytes
            # AES-256-GCM = 32 bytes
            # ChaCha20-Poly1305 = 32 bytes
            case "$method" in
                2022-blake3-aes-128-gcm|2022-blake3-aes-256-gcm)
                    if [ "$actual_len" -eq 16 ]; then
                        normalized_method="2022-blake3-aes-128-gcm"
                        expected_len=16
                    elif [ "$actual_len" -eq 32 ]; then
                        normalized_method="2022-blake3-aes-256-gcm"
                        expected_len=32
                    else
                        red "SS2022 密钥长度错误：${method} 只能使用 16 或 32 字节 PSK，当前为 ${actual_len} 字节。"
                        red "请检查节点链接或服务端配置。脚本不会写入无效的 sing-box 配置。"
                        sleep 3
                        rm -f "$decoded_file" "$raw_key_file"
                        return
                    fi
                    if [ "$normalized_method" != "$method" ]; then
                        yellow "检测到节点链接中的算法标记与实际 PSK 长度不一致。"
                        yellow "${method} + ${actual_len} 字节 PSK → 自动修正为 ${normalized_method}。"
                ...
                2022-blake3-chacha20-poly1305)
                    ...
                    if [ "$actual_len" -ne "$expected_len" ]; then
                        red "SS2022 密钥长度错误：${method} 要求 32 字节，当前为 ${actual_len} 字节。"
                        red "请检查节点链接或服务端配置。脚本不会写入无效的 sing-box 配置。"
                        sleep 3
                        rm -f "$decoded_file" "$raw_key_file"
                        return
                    ...
                    ;;
                *)
                    # 未知 2022 method 不猜测算法，避免生成不可用配置。
                    red "不支持的 SS2022 加密方式：${method}"
                    red "当前支持：2022-blake3-aes-128-gcm、2022-blake3-aes-256-gcm、2022-blake3-chacha20-poly1305"
                    sleep 3
                    rm -f "$decoded_file" "$raw_key_file"
                    return
                    ;;
            ...
            # jq 参数必须是 ASCII/Base64 字符串，绝不把原始二进制 PSK
            # 通过 shell 变量传递。
            password="$(base64 "$raw_key_file" 2>/dev/null | tr -d '\n')"
            [ -n "$password" ] || { red "SS2022 PSK Base64 编码失败。"; sleep 2; return; }
        else
            # 普通 SS 的 password 是文本，可以安全放入 shell 变量。
            decoded_text="$(cat "$decoded_file")"
            [[ "$decoded_text" == *":"* ]] || { red "格式错误：无法解析加密方式或密码。"; sleep 2; return; }
            method="${decoded_text%%:*}"
            password="${decoded_text#*:}"
        fi
    fi

    [ -z "$method" ] || [ -z "$password" ] && { red "格式错误：无法解析加密方式或密码"; sleep 2; return; }

    # 对明文 SS2022 形式再次进行长度/算法校正。
    if [[ "$method" == 2022-* ]]; then
        if [ ! -s "$raw_key_file" ]; then
            # 明文形式已经尝试将 password 解码为原始 PSK。
            printf '%s' "$password" | base64 -d > "$raw_key_file" 2>/dev/null || {
                red "SS2022 密钥不是有效的 Base64 PSK。"
                sleep 2
                rm -f "$decoded_file" "$raw_key_file"
                return
            }
        ...
        case "$method" in
            2022-blake3-aes-128-gcm|2022-blake3-aes-256-gcm)
                if [ "$actual_len" -eq 16 ]; then
                    ...
                    red "SS2022 密钥长度错误：AES-128 要求 16 字节、AES-256 要求 32 字节，当前为 ${actual_len} 字节。"
                    sleep 3
                    rm -f "$decoded_file" "$raw_key_file"
                    return
                fi
                if [ "$normalized_method" != "$method" ]; then
                    yellow "检测到算法标记与实际 PSK 长度不一致：${method} → ${normalized_method}"
                    method="$normalized_method"
                fi
                ;;
            2022-blake3-chacha20-poly1305)
    ...
    yellow "正在测试 shadowsocks 节点 ${server}:${port} (${method}) 的端口连通性..."
    if timeout 5 bash -c "echo > /dev/tcp/${server}/${port}" 2>/dev/null; then
        green "端口可达"
    else
        yellow "警告：无法探测到该端口的TCP连通性(部分服务器会屏蔽探测，不代表一定不可用)"
        reading "是否仍然添加此代理？(y/n): " force_add
        [[ ! "$force_add" =~ ^[yY]$ ]] && { yellow "已取消"; sleep 1; return; }
    fi

    [ -n "$tag_from_url" ] && tag="$tag_from_url" || tag="ss-${server}-${port}"
    jq -e --arg tag "$tag" '.outbounds[] | select(.tag == $tag)' "$outbound_file" >/dev/null 2>&1 \
        && { red "出站标签 '${tag}' 已存在"; sleep 2; return; }

    # ============================================================
    # 先备份，写入后立即检查完整配置。
    # SS2022 配置有问题时绝不直接重启 sing-box，避免所有现有节点一起掉线。
    # ============================================================
    backup_outbound="$(mktemp)"
    backup_route="$(mktemp)"
    cp -f "$outbound_file" "$backup_outbound" || {
        red "无法备份 outbounds.json，已取消操作。"
        rm -f "$backup_outbound" "$backup_route"
        sleep 2
        rm -f "$decoded_file" "$raw_key_file"
        return
    }
    cp -f "$route_file" "$backup_route" 2>/dev/null || true

    if ! jq --arg tag "$tag" --arg server "$server" --arg port "$port" \
       ...
       "$outbound_file" > "${outbound_file}.tmp"; then
        red "写入 SS 出站配置失败，未修改现有配置。"
        rm -f "$backup_outbound" "$backup_route" "${outbound_file}.tmp"
        sleep 2
        rm -f "$decoded_file" "$raw_key_file"
        return
    fi
    ...
    # 重要：添加代理出站不自动修改现有分流规则。
    # 用户应在“设置分流服务”中单独选择哪个 rule_set 使用 relay。

    check_output=$("${work_dir}/${server_name}" check -C "$conf_dir" 2>&1)
    if [ $? -ne 0 ]; then
        red "SS2022 已添加，但 sing-box 配置检查失败："
        echo "$check_output"
        yellow "正在自动回滚本次修改..."
        cp -f "$backup_outbound" "$outbound_file"
        cp -f "$backup_route" "$route_file" 2>/dev/null || true
        rm -f "$backup_outbound" "$backup_route"
        red "已回滚，现有节点配置未被破坏。"
        sleep 3
        rm -f "$decoded_file" "$raw_key_file"
    ...
    if [ $restart_status -ne 0 ]; then
        red "sing-box 重启失败，正在自动回滚本次 SS 出站修改..."
        cp -f "$backup_outbound" "$outbound_file"
        cp -f "$backup_route" "$route_file" 2>/dev/null || true
        rm -f "$backup_outbound" "$backup_route"
        restart_singbox >/dev/null 2>&1
    ...
    # 再确认一次服务确实处于 active 状态。
    if command_exists systemctl; then
        if ! systemctl is-active --quiet sing-box; then
            red "sing-box 重启后未处于运行状态，正在自动回滚..."
            cp -f "$backup_outbound" "$outbound_file"
            cp -f "$backup_route" "$route_file" 2>/dev/null || true
            rm -f "$backup_outbound" "$backup_route"
            restart_singbox >/dev/null 2>&1
    ...
    rm -f "$backup_outbound" "$backup_route"
    rm -f "$decoded_file" "$raw_key_file"
    green "\n${tag} (${method}) shadowsocks 代理出站已添加"
    green "不会自动修改现有分流规则，请在“设置分流服务”中选择需要走 ${tag} 的规则集。\n"
    sleep 2; warp_manage
}
add_socks5_proxy() {
    clear
    reading "请输入代理URL (支持socks://,socks5://,http://,ss:// 支持v2rayN导出的节点链接): " proxy_url
    [ -z "$proxy_url" ] && { red "输入为空！"; sleep 1; return; }

    proto=$(echo "$proxy_url" | grep -oP '^[a-zA-Z0-9]+(?=://)')
    [[ ! "$proto" =~ ^(socks5|socks|http|ss)$ ]] && { red "不支持的协议"; sleep 2; return; }

    if [[ "$proto" == "ss" ]]; then
        add_ss_outbound "$proxy_url"
        return
    fi
    ...
    if [[ "$host_port" =~ ^\[([0-9a-fA-F:]+)\]:([0-9]+)$ ]]; then
        server="${BASH_REMATCH[1]}"; port="${BASH_REMATCH[2]}"
    else
    ...
    # 拼URL用的地址：IPv6需要重新套上方括号，纯IPv4/域名不用
    local url_server="$server"
    [[ "$server" == *:* ]] && url_server="[${server}]"
    ...
    if [ "$is_local" = true ]; then
        ...
        yellow "检测到本地代理 ${check_proto}://${url_server}:${port}，跳过外部API检测，正在用curl测试连通性..."
        local curl_proxy_url="${check_proto}://${proxy_auth}${url_server}:${port}"
        local test_result
        ...
        # 远程代理：调用外部 API 检测
        yellow "正在测试代理 ${check_proto}://${url_server}:${port} ..."
        local api_response
        api_response=$(curl -s --max-time 8 -G \
            --data-urlencode "proxy=${check_proto}://${proxy_auth}${url_server}:${port}" \
    ...
    # 添加 Socks/HTTP 出站时也不覆盖已有分流规则。
    # 需要使用它时，请在“设置分流服务”中单独选择对应出站。
    ensure_sniff_rule

    check_output=$(${work_dir}/${server_name} check -C "$conf_dir" 2>&1)
    if [ $? -ne 0 ]; then
        red "代理配置检查失败，已保留原有分流配置。"
        echo "$check_output"
        sleep 2
        return
    fi
...
# 域名证书管理：交互式设置/更换/恢复 hy2+tuic 使用的证书
manage_domain_cert() {
    check_singbox &>/dev/null
    if [ $? -eq 2 ]; then
    ...
    clear
    green "=== 域名证书管理 ===\n"
    if [ -f "${work_dir}/domain.txt" ] && [ -s "${work_dir}/domain.txt" ]; then
        green "当前证书域名: ${purple}$(cat "${work_dir}/domain.txt")${re}"
        green "证书颁发者: ${purple}$(openssl x509 -in "${work_dir}/cert.pem" -noout -issuer 2>/dev/null | sed 's/issuer=//')${re}\n"
    else
        yellow "当前使用自签证书 (bing.com)，未绑定真实域名\n"
    fi

    green "1. 设置/更换域名证书 (申请Let's Encrypt真实证书)"
    red   "2. 恢复自签证书"
    purple "0. 返回主菜单"
    skyblue "------------"
    reading "请输入选择: " dc_choice

    case "$dc_choice" in
        1)
            reading "请输入你的域名 (需已提前解析到本机公网IP): " new_domain
            [ -z "$new_domain" ] && { red "域名不能为空"; sleep 1; return; }

            issue_domain_cert "$new_domain"

            if [ -f "${work_dir}/domain.txt" ] && [ "$(cat "${work_dir}/domain.txt")" == "$new_domain" ]; then
                jq --arg d "$new_domain" \
                   '(.inbounds[] | select(.type=="hysteria2") | .masquerade) = ("https://" + $d)' \
                   "${conf_dir}/inbounds.json" > "${conf_dir}/inbounds.json.tmp" \
                   && mv "${conf_dir}/inbounds.json.tmp" "${conf_dir}/inbounds.json"

                sed -i -E "/^hysteria2:\/\// s/sni=[^&]*/sni=${new_domain}/" "${client_dir}"
                sed -i -E "/^hysteria2:\/\// s/insecure=[01]/insecure=0/" "${client_dir}"
                sed -i -E "/^hysteria2:\/\// s/\&pinSHA256=[^&]*//" "${client_dir}"
                sed -i -E "/^tuic:\/\// s/sni=[^&]*/sni=${new_domain}/" "${client_dir}"
                sed -i -E "/^tuic:\/\// s/allow_insecure=[01]/allow_insecure=0/" "${client_dir}"

                restart_singbox
                green "\n域名证书已切换为 ${new_domain}，节点信息已同步更新，请重新导入订阅\n"
            fi
            ;;
        2)
            issue_domain_cert ""
            new_fp=$(openssl x509 -noout -fingerprint -sha256 -in "${work_dir}/cert.pem" | cut -d'=' -f2 | sed 's/:/%3A/g')

            jq '(.inbounds[] | select(.type=="hysteria2") | .masquerade) = "https://bing.com"' \
                "${conf_dir}/inbounds.json" > "${conf_dir}/inbounds.json.tmp" \
                && mv "${conf_dir}/inbounds.json.tmp" "${conf_dir}/inbounds.json"

            sed -i -E "/^hysteria2:\/\// s/sni=[^&]*/sni=www.bing.com/" "${client_dir}"
            sed -i -E "/^hysteria2:\/\// s/insecure=[01]/insecure=1/" "${client_dir}"
            sed -i -E "/^hysteria2:\/\// s/\&pinSHA256=[^&]*//" "${client_dir}"
            sed -i -E "/^hysteria2:\/\// s/insecure=1/insecure=1\&pinSHA256=${new_fp}/" "${client_dir}"
            sed -i -E "/^tuic:\/\// s/sni=[^&]*/sni=www.bing.com/" "${client_dir}"
            sed -i -E "/^tuic:\/\// s/allow_insecure=[01]/allow_insecure=1/" "${client_dir}"
...
menu() {
    ...
    green "Github地址: ${purple}https://github.com/tbjpvn/sb-re${re}\n"
        ...
        while true; do
            ...
            case "${choice}" in
                ...
                10)
                    ...
                    need_pause=false
                    ;;
                11) manage_domain_cert; need_pause=false ;;
                0)  exit 0 ;;       
                *)
                    red "无效的选项，请输入 0-11"
                    need_pause=true
