...
restart_singbox(){
    # 统一确保所有代理入站启用域名嗅探，供 Gemini/Claude/OpenAI 等 rule_set 使用
    local _ib="${conf_dir}/inbounds.json"
    if [ -f "$_ib" ]; then
        jq '(.inbounds[] | select(.type == "vless" or .type == "vmess" or .type == "hysteria2" or .type == "tuic" or .type == "socks" or .type == "anytls" or .type == "shadowsocks")) |= (. + {"sniff": true})' "$_ib" > "${_ib}.tmp" && mv "${_ib}.tmp" "$_ib"
    fi
...
warp_manage() {
    ...
    echo ""
    green "1. 设置分流服务 (未添加代理出站则使用WARP)"
    skyblue "----------------------"
    red "2. 删除分流服务"
    skyblue "--------------"
    green "3. 添加 SOCKS5/HTTP/SS2022 出站"
    skyblue "----------------------"
    red "4. 删除 SOCKS5/HTTP/SS2022 出站"
    skyblue "----------------------"
    purple "0. 返回主菜单"
    skyblue "------------"
    purple "00. 退出脚本"
...
set_global_outbound() {
    ...
    if [ ${#proxy_tags[@]} -eq 0 ]; then
        yellow "\n当前没有可用的代理出站。"
        yellow "请先返回 → 设置分流服务 → 添加 SOCKS5/HTTP/SS2022 出站，再设置全局代理。\n"
...
add_proxy_outbound() {
    ...
    green "2. HTTP"
    green "3. Shadowsocks-2022 (SS2022)"
    purple "0. 返回"
    reading "请输入选择: " proxy_choice
    [ "$proxy_choice" = "0" ] && { warp_manage; return; }
    ...
    case "$proxy_choice" in
        1) allowed='^(socks5|socks)$'; outbound_type="socks" ;;
        2) allowed='^(http)$'; outbound_type="http" ;;
        *) red "无效选择"; sleep 1; add_proxy_outbound; return ;;
    esac

    reading "请输入代理URL: " proxy_url
    [ -z "$proxy_url" ] && { red "输入为空！"; sleep 1; return; }
    proto=$(echo "$proxy_url" | grep -oP '^[a-zA-Z0-9]+(?=://)')
    [[ "$proto" =~ $allowed ]] || { red "协议与所选类型不匹配"; sleep 2; return; }
    ...
    if [ -n "$user_pass" ]; then
        ...
        if [ -n "$decoded" ] && [[ "$decoded" == *":"* ]]; then user="${decoded%%:*}"; password="${decoded#*:}"
        elif [[ "$user_pass" == *":"* ]]; then user="${user_pass%%:*}"; password="${user_pass#*:}"
        else user="$user_pass"; fi
    fi
    if [[ "$host_port" =~ ^\[(.*)\]:([0-9]+)$ ]]; then server="${BASH_REMATCH[1]}"; port="${BASH_REMATCH[2]}"
    else server="${host_port%:*}"; port="${host_port##*:}"; fi
    [[ -n "$server" && "$port" =~ ^[0-9]+$ && "$port" -ge 1 && "$port" -le 65535 ]] || { red "格式错误：服务器或端口无效"; sleep 2; return; }
    ...
    if [ -n "$user" ] && [ -n "$password" ]; then
      jq --arg type "$outbound_type" --arg tag "$tag" --arg server "$server" --argjson port "$port" --arg user "$user" --arg password "$password" '.outbounds += [{"type":$type,"tag":$tag,"server":$server,"server_port":$port,"username":$user,"password":$password}]' "$outbound_file" > "${outbound_file}.tmp" && mv "${outbound_file}.tmp" "$outbound_file"
    else
      jq --arg type "$outbound_type" --arg tag "$tag" --arg server "$server" --argjson port "$port" '.outbounds += [{"type":$type,"tag":$tag,"server":$server,"server_port":$port}]' "$outbound_file" > "${outbound_file}.tmp" && mv "${outbound_file}.tmp" "$outbound_file"
    fi
    restart_singbox
    green "\n${tag} 代理出站已添加\n"
    sleep 1; warp_manage
...
add_ss2022_outbound() {
    clear
    green "=== 添加 Shadowsocks-2022 分流出站 ==="
